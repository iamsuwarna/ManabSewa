...
class ClothDonationPage extends StatefulWidget {
  @override
  _ClothDonationPageState createState() => _ClothDonationPageState();
}

class _ClothDonationPageState extends State<ClothDonationPage> {
  // creating global variables
  final _clothdonationFormKey = GlobalKey<FormState>();
  final TextEditingController _clothdescription = TextEditingController();
  final TextEditingController _clothcount = TextEditingController();
...
//for picking up images from phone's gallery
  ...
  Widget build(BuildContext context) {
      ...
      Navigator.push(
          ...
          MaterialPageRoute(
              builder: (context) => NGONear(
                    ...
                    description: _clothdescription.text,
                    count: _clothcount.text.length,
    ...
    //
    print('this is cloth address ${_selectedAddress}');
          ...
          child: Form(
            ...
            child: Column(
              children: <Widget>[
                ...
                CustomTextField(
                  controller: _clothdescription,
                  hintText: "Clothe's Description",
                ),
                SizedBox(
                  height: 20,
                ),
                FoodCountTextField(
                    CountName: 'Cloth-Count', controller: _clothcount),
                ...
                CustomButton(
                  ...
                  onTap: () {
                    if (_clothdonationFormKey.currentState!.validate()) {
                      if (images.isEmpty) {
                        showSnackBar(context,
                            "Please add the image of an item to donate!");
                      } else if (_clothcount.text.isEmpty) {
                        showSnackBar(
                            context, "Please specify the cloth count!");
                      } else {
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: ((context) => NGONear(
                                    donorName: donor.name,
                                    description: _clothdescription.text,
                                    count: int.parse(_clothcount.text),
                                    address: _selectedAddress,
                                    type: 'Cloth-Donation',
                                    areasOfInterest: _selectedAreaOfInterest,
                                    imagee: images))));
